...
public:
  ...
  double compute_inflow_difference(const GridType& grid,
    ...
    const auto only_on_left_plane = [=](const ViewType&, const IntersectionType& it){ return it.boundary()
          && DSC::FloatCmp::eq(it.geometry().center()[0], 0.); };
  ...
  virtual double eval() {
    ...
    const Multiscale::CommonTraits::SpaceType coarse_space(
          PartViewType::create(*coarse_grid, Multiscale::CommonTraits::st_gdt_grid_level));
    std::unique_ptr<Multiscale::LocalsolutionProxy> msfem_solution(nullptr);

    Multiscale::LocalGridList localgrid_list(coarse_space);
    Multiscale::Elliptic_MsFEM_Solver().apply(coarse_space, msfem_solution, localgrid_list);
    auto fine_grid = Multiscale::make_fine_grid(coarse_grid, true);
    const Multiscale::CommonTraits::SpaceType fine_space(
          PartViewType::create(*fine_grid, Multiscale::CommonTraits::st_gdt_grid_level));
    Multiscale::Elliptic_FEM_Solver fem(fine_grid);
    const auto& fine_fem_solution = fem.solve();

    //sollte eigentlich auch ohne den umweg der projection auf das feine gitter möglich sein
    Multiscale::CommonTraits::DiscreteFunctionType projected_msfem_solution(fine_space, "MsFEM_Solution");
    Multiscale::MsFEMProjection::project(*msfem_solution, projected_msfem_solution, msfem_solution->search());
    return compute_inflow_difference(fine_grid, projected_msfem_solution, fine_fem_solution);
  }
};

//! workaround for https://github.com/wwu-numerik/dune-stuff/issues/42
void set_config_values(const std::vector<std::string>& keys, const std::vector<std::string>& values) {
